            ...
            for n, line in enumerate(f):
                ...
                if _pwds:
                    if len(_pwds) == 3 and _counter == instance + 1:
                        tasks.append(RPQ_Task(_t_counter, _pwds[0], _pwds[1], _pwds[2]))
                        _t_counter += 1
                    elif len(_pwds) < 3:
                        _counter += 1
        instanceRPQ.tasks = tasks
        instanceRPQ.tasks_number = len(tasks)
        return instanceRPQ
...
if __name__ == '__main__':
    ...
    test_instance = RPQ_Instance.load_from_file('rpq.data.txt', 3)
